...
const InvoiceApp = () => {
  ...
  const calculateFields = (newData: Partial<Invoice>) => {
    ...
    return { discount, tax, totalPrice: parseFloat(totalPrice.toFixed(3)) };
  };
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (parseFloat(value) < 0) {
      return; // Prevent negative values from being set
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingInvoice) {
      ...
      setInvoices(
        invoices.map((inv) => (inv.id === editingInvoice ? formData : inv))
      );
